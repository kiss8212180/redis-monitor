...
<script>
	...
	function flushAll(){
		...
		$.getJSON(window.contextPath + '/flushall.htm', function (data) {
	...
	function flushDB(){
		...
		$.getJSON(window.contextPath + '/flushDb.htm', function (data) {
			if(data.statu.toLowerCase() == 'ok'){
				message('flushDB操作成功') ;
			} else {
				message('flushDB操作失败') ;
...
<div class="navbar navbar-inverse">
	<div class="navbar-inner">
		<div class="container">
			...
			</a> <a class="brand" href="<c:url value="/"/>">Angel</a>
			<div class="nav-collapse">
				<ul class="nav">
					<li class="dropdown"><a href="#" class="dropdown-toggle"
						...
						<ul class="dropdown-menu">
						   <c:forEach var="rs" items="${redisServerList }">
							   <li class=""><a
									   href="<c:url value="/index.htm?uuid=${rs.uuid }"/>">${rs.description }</a></li>
						   </c:forEach>
					...
					<li><a href="<c:url value="/keys.htm"/>">keys</a></li>
					<li><a href="<c:url value="/config/toConfigDetail.htm"/>">redis-config.xml</a></li>
					<li><a href="<c:url value="/server/redisServer.htm"/>">redis节点管理</a></li>
					<li><a href="<c:url value="/monitor.htm"/>">redis实时监控</a></li>
				</ul>
